...
import pytest
from thefeck.entrypoints.alias import _get_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='feck', )
    mocker.patch('thefeck.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('thefeck.entrypoints.alias.shell', shell)
