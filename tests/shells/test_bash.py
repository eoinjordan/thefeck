...
import os
import pytest
from thefeck.shells import Bash
...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thefeck.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias feck=\'eval $(thefeck $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'feck': 'eval $(thefeck $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}
    ...
    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('feck')
        assert "feck () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=feck" in alias
